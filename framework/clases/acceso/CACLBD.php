...
class CACLBD extends CACLBase{
    ...
    public function esActivado($nick, $contrasena)
    {
        $prefijo="TT";
        if (!$this->valido())
            return false;
            ...
            if (!$this->existeUsuario($nick))
                ...
                $sentencia = "SELECT a.nombre ".
                    "      FROM acl_usuarios a ".
                    "       INNER JOIN usuarios u on a.nick=u.nick ".
                    "      WHERE a.nick='$nick' and a.contrasena=md5('$contrasena')".
                    "         and u.activado";
                $resultado = $this->_mysqli->query($sentencia);
                if (!$resultado)  //se ha producido un error
                    return false;
